From Frederic.Lepied@sugix.frmug.org Tue Aug 19 11:25:21 1997
Xref: CERN.ch gnu.emacs.sources:6228
Path: CERN.ch!EU.net!howland.erols.net!news.apfel.de!univ-lyon1.fr!jussieu.fr!rain.fr!francenet.fr!itesec!frmug.fr.net!sugix.frmug.fr.net!not-for-mail
From: Frederic Lepied <Frederic.Lepied@sugix.frmug.org>
Newsgroups: gnu.emacs.sources
Subject: Re: cvs.el 1.23
Date: 13 Aug 1997 04:34:35 +0200
Organization: Private Linux Node
Lines: 1466
Message-ID: <oarabyhkuc.fsf@sugix.frmug.org>
References: <oaen818iu1.fsf@sugix.frmug.org>
Mime-Version: 1.0
Content-Type: text/plain; charset=ISO-8859-1
Content-Transfer-Encoding: 8bit
X-Server-Date: 13 Aug 1997 02:34:58 GMT
To: Elisp Archive Maintainer <elisp-archive@cis.ohio-state.edu>
X-No-Archive: yes
X-Attribution: Fred
X-Face: k+,iUumE^nDx[DExj+{Axb>'\HVHDb+ZUSG6"Sx(-u!j}Vj'k=qte)8R.p7km[sfkZ{+UqL
 u9,P[`U2;yOh2z>1kL%u)0g#|-=>NT?b5[ZoQ`7KY:sJ_L|t;LvT@}g,6zg*F;6"l^.5E-Z]/W4!V"
 e{u
X-Newsreader: Gnus v5.3/Emacs 19.34
X-IMAPbase: 1200940249 1
Status: O
X-Status: 
X-Keywords:                       
X-UID: 1

What's new from 1.23:

	* should work with XEmacs (tested with 19.15)
	* atfter the merge process with ediff, write the *ediff-merge* buffer
to the corresponding file.

;; cvs.el --- Light cvs support for emacs (ediff + msb + dired + mode line)
;;
;; Copyright (C) 1995-1997 Frederic Lepied <Frederic.Lepied@sugix.frmug.org>
;;
;; Author: Frederic Lepied <Frederic.Lepied@sugix.frmug.org>
;; Version: $Id: cvs.el,v 1.1 2005/08/19 10:32:49 sashby Exp $
...
;; $Date: 2005/08/19 10:32:49 $|$Revision: 1.1 $|~/modes/cvs.el.gz|
;;
...
;;=============================================================================
(defconst cvs:version "$Id: cvs.el,v 1.1 2005/08/19 10:32:49 sashby Exp $"
...
Help to admin CVS controlled files :
  ...
  (setq cvs-mode (if (null arg) t (car arg)))
  (if cvs-mode
      (progn
...
;;=============================================================================
(defun cvs-status (&optional file)
...
(defun cvs-flush ()
  ...
  (mapcar (function (lambda (c)
			  ...
			  (save-excursion
			    ...
			    (setq cvs:mark nil)))))
	  cvs:commit-list)
  (if (eq cvs:marked-list cvs:commit-list)
...
(defun cvs-do-commit ()
  ...
  (let* ((filename (make-temp-name (concat (or (and cvs-temp-dir
					       ...
					       "/tmp/")
					   "cvs")))
	 (dedicated (window-dedicated-p (get-buffer-window (current-buffer))))
...
(defun cvs:status-sentinel (process event)
  ...
  (cvs:display-temp-buffer (get-buffer "*CVS-Statuslist*") "Statuslist")
...
(defun cvs-status-process (&optional files)
  ...
  (save-excursion
    (let ((dir default-directory)
	  (buf	(get-buffer-create "*CVS-Statuslist*")))
      (set-buffer buf)
	...
	(insert (format "%s\n" "Status of Files"))
	(insert "==================================================\n"))))
  (let* ((args (cons "-d" (if cvs-root (append (list "-d" cvs-root) files) files)))
	 (proc (eval (append
		      (list 'start-process "cvs status" "*CVS-Temp*"
...
(defun cvs:add-msb-submenu ()
  ...
  (setq msb-menu-cond (append (list '((and (boundp 'cvs-mode)
					   cvs-mode
					   'multi)
				      1015
				      "CVS (%d)")
...
(define-key cvs-dired:map "\C-cvs" 'cvs-dired-status)
...
(defun cvs-dired-status ()
  "Call `cvs-status' on current file."
  (interactive)
  (cvs-status (dired-get-filename)))
