...
;; auto-fill-mode and the commands fill-paragraph and
;; lisp-fill-paragraph.  The chief improvement is that the
;; beginning of a line to be filled is examined and, based on
;; information gathered, an appropriate value for fill-prefix is
;; constructed.  Also the boundaries of the current paragraph are
;; located.  This occurs only if the fill prefix is not already
;; non-nil.
;;
...
filladapt uses this table to build a tokenized representation of the
beginning of the current line.  Each REGEXP is matched against the
beginning of the line until a match is found.  The corresponding SYM
is added to the list, point is moved to (match-end 0) and the process
is repeated.  The process ends when there is no REGEXP in the table
that matches what is at point.")
...
match what is at point then the token generator gives up and
...
(defvar filladapt-function-table
  (list (cons 'fill-paragraph (symbol-function 'fill-paragraph))
	(cons 'lisp-fill-paragraph (symbol-function 'lisp-fill-paragraph))
...
token with the greater end column is not moved forward, which allows
its current token to be matched the next token in the other list in
the next iteration of the matching loop.

All tokens must be matched in order for the lists to be consiered
matching."
  (let ((matched t)
	(done nil))
  ...
  (save-excursion
	...
	(if paragraph
		...
		(while (not done)
	      ...
	      ;; Multiple spaces at the beginning of the first
	      ;; line of a hanging list paragraph get squashed by
	      ;; fill-paragraph.  We kludge around this by hiding
	      ;; the real line begining and creating a fake one
	      ;; that's all spaces.  fill-paragraph won't touch a
	      ;; line prefix of all spaces so we win.  The post
	      ;; hook cleans up the spaces after fill-paragraph
	      ;; has been called.
		  ...
		  (let (col)
		    ...
		    (insert-char (string-to-char " ") col)
